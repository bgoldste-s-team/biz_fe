  ...
  const highlightText = (text, query) => {
    ...
    while (matchCount < MAX_MATCHES) {
      const match = remainingText.match(
        new RegExp(
          `(.{0,${SNIPPET_LENGTH}})(${query})(.{0,${SNIPPET_LENGTH}})`,
          "i"
        )
      );
  ...
  const handleSearch = () => {
    ...
    pages.forEach((page) => {
      page.content_blocks.forEach((block) => {
        const highlightedHeader = block.header
          ? highlightText(block.header, searchTerm)
          : null;
        const highlightedSubheader = block.subheader
          ? highlightText(block.subheader, searchTerm)
          : null;
        const highlightedBody = block.body
          ? highlightText(block.body, searchTerm)
          : null;
          ...
          results.map((result, index) => (
            <div key={index} className="card bordered">
              ...
              <div className="card-body">
                ...
                <div
                  dangerouslySetInnerHTML={{ __html: result.blockHeader || "" }}
                ></div>
                <div
                  dangerouslySetInnerHTML={{
                    __html: result.blockSubheader || "",
                  }}
                ></div>
                <p
                  dangerouslySetInnerHTML={{ __html: result.blockBody || "" }}
                ></p>
              </div>
              <Link
                className="btn btn-secondary"
                href={`/${result.pageSlug}#${result.blockOrder}`}
              >
                View More
              </Link>
            </div>
